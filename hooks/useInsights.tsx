...
const generateCategoryBreakdowns = (
	...
	const subscriptionsByCategory = subscriptions.reduce(
		(acc, sub) => {
	...
	const subscriptionBreakdown = Object.entries(subscriptionsByCategory).map(([name, value]) => ({
		name,
		value,
	}));
...
const generateAccountInsights = (
	...
	return accounts.map(account => {
		...
		const accountSubscriptions = subscriptions.filter(sub => sub.userId === account.id);
		const totalAmount = [...accountExpenses, ...accountSubscriptions.map(sub => ({ amount: sub.amount }))].reduce(
...
const updateSubscription = async (id: string, subscription: Partial<Subscription>): Promise<Subscription> => {
	// Simulate API call
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			const existingSubscription = MOCK_SUBSCRIPTIONS.find(sub => sub.id === id);
...
const deleteSubscription = async (id: string): Promise<void> => {
	...
	return new Promise((resolve, reject) => {
		setTimeout(() => {
			const subscriptionIndex = MOCK_SUBSCRIPTIONS.findIndex(sub => sub.id === id);
...
const createExpense = async (expense: Omit<Expense, 'id' | 'logoUrl'>): Promise<Expense> => {
	// Simulate API call
	return new Promise(resolve => {
		setTimeout(() => {
			const newExpense = {
...
export function useInsights() {
	...
	const insightSummary = generateInsightSummary(subscriptions, expenses, gmailAccounts);
	...
	const accountInsights = generateAccountInsights(gmailAccounts, subscriptions, expenses);
	...
	useEffect(() => {
		const fetchData = async () => {
			setIsLoading(true);
			try {
				const [fetchedSubscriptions, fetchedExpenses] = await Promise.all([getSubscriptions(), getExpenses()]);
	...
	const connectAccount = async (email: string, name: string) => {
		...
		try {
			...
			const newAccount: GmailAccount = {
				...
				avatarUrl: `https://ui-avatars.com/api/?name=${name.replace(/\s+/g, '+')}&background=8B5CF6&color=fff`,
			};
	...
	const disconnectAccount = async (accountId: string) => {
		...
		try {
			...
			setGmailAccounts(prevAccounts =>
				prevAccounts.map(account =>
					account.id === accountId ? { ...account, connected: false, lastSynced: null } : account,
				),
			);
	...
	const updateSubscriptionData = async (id: string, subscription: Partial<Subscription>) => {
		...
		try {
			...
			setSubscriptions(prev => prev.map(sub => (sub.id === id ? { ...sub, ...updatedSubscription } : sub)));
